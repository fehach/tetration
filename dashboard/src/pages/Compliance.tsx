...
import { api, streamComplianceSummary } from "../api/client";
import type {
  ComplianceScope,
  PciAssessment,
  PciRequirement,
...
export function CompliancePage() {
  ...
  const [summaryState, setSummaryState] = useState<"idle" | "streaming" | "done" | "error">(
    "idle",
  );
  ...
  const runAssessment = async () => {
    ...
    setSummary("");
    setSummaryState("idle"); // a new assessment needs a fresh summary
    try {
  ...
  const generateSummary = async () => {
    ...
    try {
      await streamComplianceSummary(
        assessment,
        (event) => {
          if (event.type === "text") setSummary((prev) => prev + event.chunk);
          if (event.type === "error") {
            setSummary(event.message);
            setSummaryState("error");
          }
          if (event.type === "done") {
            setSummaryState((state) => (state === "error" ? "error" : "done"));
          }
        },
        controller.signal,
      );
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          {assessment && (
            <button
              ...
              className="inline-flex items-center gap-2 rounded border border-surface-border px-4 py-2 text-sm font-semibold text-muted transition hover:bg-surface-hover hover:text-white disabled:opacity-50"
            >
              <Sparkles className="h-4 w-4 text-cisco-blue" aria-hidden />
              {summaryState === "streaming" ? "Generating…" : "Generate executive summary"}
            </button>
          )}
        </div>
      ...
      {summaryState !== "idle" && (
        <SummaryPanel text={summary} state={summaryState} model={model} />
      )}
...
function SummaryPanel({
  ...
  return (
    <Card className="border-l-4 border-l-cisco-blue">
      <div className="mb-2 flex flex-wrap items-center gap-2.5">
        <Sparkles className="h-4 w-4 text-cisco-blue" aria-hidden />
        <h2 className="text-base font-semibold tracking-tight">Executive summary</h2>
        <span className="inline-flex items-center rounded-full border border-cisco-blue/40 bg-cisco-blue/10 px-2.5 py-0.5 text-[11px] font-semibold text-muted">
          Generated by Claude{model ? ` · ${model}` : ""}
        </span>
      </div>
      {state === "error" ? (
        <p className="text-sm text-cisco-red">{text || "Summary generation failed."}</p>
      ) : (
        <p className="max-w-prose whitespace-pre-wrap text-sm leading-relaxed text-muted">
          {text}
          {state === "streaming" && (
            <span
              className="ml-0.5 inline-block h-3.5 w-1.5 animate-pulse bg-cisco-blue align-[-2px]"
              aria-hidden
            />
          )}
        </p>
      )}
      <p className="mt-3 flex items-start gap-1.5 text-[11px] text-muted-dim">
        <Info className="mt-0.5 h-3 w-3 shrink-0" aria-hidden />
        AI-generated from the deterministic assessment — review before acting. The scored checks
        remain the source of truth for audit evidence.
      </p>
    </Card>
  );
}
